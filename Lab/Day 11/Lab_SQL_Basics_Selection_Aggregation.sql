...
SELECT trans_id, date, amount FROM bank.trans
...
SELECT district_id, COUNT(client_id) AS client_count FROM bank.client
WHERE district_id < 10
GROUP BY 1
ORDER BY 1;

-- Query 14 - In the card table, how many cards exist for each type? Rank the result starting with the most frequent type.
SELECT type, COUNT(card_id) AS card_count FROM bank.card
GROUP BY 1
ORDER BY 2 DESC;
...
SELECT date, COUNT(loan_id) AS number_of_loans_issued FROM bank.loan
WHERE date < 930907
GROUP BY 1
ORDER BY 1 DESC;
...
SELECT date, duration, COUNT(loan_id) AS number_of_loans_issued FROM bank.loan
...
SELECT
	DISTINCT account_id,
	(SELECT FLOOR(SUM(amount)) FROM bank.trans WHERE account_id = 396 AND type = 'PRIJEM') AS incoming,
    (SELECT FLOOR(SUM(amount)) FROM bank.trans WHERE account_id = 396 AND type = 'VYDAJ') AS outgoing,
    ((SELECT FLOOR(SUM(amount)) FROM bank.trans WHERE account_id = 396 AND type = 'PRIJEM') - (SELECT FLOOR(SUM(amount)) FROM bank.trans WHERE account_id = 396 AND type = 'VYDAJ')) AS difference
FROM bank.trans
WHERE account_id = 396;
